app.controller('Rules1Controller', function($cookies, $scope, $http, $uibModal, $routeParams, HTTPFactory, IndexOverlayFactory) {
    $scope.$parent.menu_selected = 'manage_rule';
    ...
    $scope.loadList = function(){
        ...
        var params = {'doc_type' : 'rule', 'doc_level' : 1, 'condition' : $scope.condition};
    ...
    $scope.loadSubList = function(parent_id){
        ...
        var params = {'doc_type' : 'rule', 'doc_level' : 2, 'parent_id' : parent_id};
        HTTPFactory.clientRequest('docs/list', params).then(function (result) {
            if (result.data.STATUS == 'OK') {
...
app.controller('Rules2Controller', function($cookies, $scope, $http, $uibModal, $routeParams, HTTPFactory, IndexOverlayFactory) {
    ...
    $scope.loadSubList = function(parent_id){
        ...
        var params = {'doc_type' : 'rule', 'doc_level' : 3, 'parent_id' : parent_id};
        HTTPFactory.clientRequest('docs/list', params).then(function (result) {
            if (result.data.STATUS == 'OK') {
                $scope.SubDataList = result.data.DATA.List;
            }
...
app.controller('Rules3Controller', function($cookies, $scope, $http, $uibModal, $routeParams, HTTPFactory, IndexOverlayFactory) {
    $scope.$parent.menu_selected = 'manage';
    IndexOverlayFactory.overlayShow();
    var $user_session = sessionStorage.getItem('user_session');

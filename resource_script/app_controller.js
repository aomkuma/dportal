...
app.controller('HomeController', function($scope, $location, $sce, IndexOverlayFactory, HomeFactory, ReserveRoomFactory) {
	//sessionStorage.removeItem('user_session');
    $scope.$parent.menu_selected = '';
    var $user_session = sessionStorage.getItem('user_session');
    $scope.$parent.TotalLogin = sessionStorage.getItem('TotalLogin');
    ...
    $scope.getNewFeed = function(){
        HomeFactory.getNewsFeed($scope.currentUser.RegionID).then(function(result){
            $scope.newsSlides = result.data.DATA;
        });
    }

    $scope.setIntervalSlide = function(){
        setInterval(function(){
            if($scope.countSlide == $scope.newsSlides.length){
                $scope.countSlide = 0;
                $scope.currentSlide = $scope.newsSlides[0];
                
            }else{
                $scope.currentSlide = $scope.newsSlides[1];
            }
    ...
    $scope.setCurrentSlide = function(index, slide){
        //console.log(slide);
        $scope.countSlide = index;
    ...
    $scope.regionSelected = $scope.currentUser.RegionID;
    ...
    $scope.loadReserveList = function(regionID){
        ...
        //console.log(regionID, $scope.dateSelected);
        IndexOverlayFactory.overlayShow();
    ...
    $scope.getStatus = function(currentTime, ReserveList, ownerID){
        ...
        for(var i=0; i < ReserveList.length; i++){
            ...
            if((reserveStartDate.getTime() == curDateTime.getTime()) || (reserveEndDate.getTime() > curDateTime.getTime() && curDateTime.getTime() > reserveStartDate.getTime())){
                ...
                }else if(ownerID == ReserveList[i].CreateBy && ReserveList[i].ReserveStatus == 'Request'){
    ...
    $scope.getCalendar = function(){
        HomeFactory.getCalendar().then(function(result){
            $scope.Calendar = result.data.DATA;
            $scope.Calendar.CalendarUrl = $sce.trustAsResourceUrl($scope.Calendar.CalendarUrl);
        });
    }
...
app.controller('RoomOverviewController', function($scope, $location, $compile, IndexOverlayFactory, ReserveRoomFactory, RegionFactory) {
    ...
    if($user_session != null){
        ...
        window.location.replace('#/logon/roomconference');
	}
